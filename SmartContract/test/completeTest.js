...
contract('eVote', async (accounts) => {
    ...
    const v = 27
    const r = '0xb1e6929a7bcc8bfae8d8f69c27b7e6c5b2a5da979e8afed0f38e8dcde55f9c27'
    const s = '0x27b5af763c446acd0f2e21d487fdd9f4a3c1f4e5d418e0e63f642387e0d638e8'
    it('Generate Testing Data', async () => {
        data = await genTestData(nVoters)
        assert(data.length == nVoters, "Error: Number of voters is not equal to the number of generated testing data")
        let encryptedVotes = [];
        ...
        const eligableVoters = accounts.slice(1, accounts.length - 1)
        assert(eligableVoters.length === nVoters)
        usersMerkleTree = new MerkleTree(eligableVoters)
    }).timeout(90e6);
    ...
    it('Register public keys for elligable users except the last one', async () => {
        for (let i = 0; i < data.length - 1; i++) {
            ...
            console.log(`sender[${i}]`, JSON.stringify({
                sender, v, r, s, _pubKey: data[i].publicKey,
                proof_a: data[i].publicKeyProof.a,
                proof_b: data[i].publicKeyProof.b,
                proof_c: data[i].publicKeyProof.c,
                _merkleProof,
            }, null, 4))

            tx = await eVoteInstance.register(sender, v, r, s, data[i].publicKey, data[i].publicKeyProof.a, data[i].publicKeyProof.b, data[i].publicKeyProof.c, _merkleProof, { from: accounts[i + 1] })
    ...
    it('Throw an error if non-elligable user tries to vote', async () => {
        ...
        try {
            ...
            await eVoteInstance.register(sender, v, r, s, data[0].publicKey, data[0].publicKeyProof.a, data[0].publicKeyProof.b, data[0].publicKeyProof.c, _merkleProof, { from: accounts[accounts.length - 1] })
        } catch (err) {
    ...
    it('Throw an error if elligable user provides invalid DL proof to vote', async () => {
        ...
        try {
            const sender = accounts[i + 1]
            await eVoteInstance.register(sender, v, r, s, data[i].publicKey, data[i - 1].publicKeyProof.a, data[i].publicKeyProof.b, data[i].publicKeyProof.c, _merkleProof, { from: accounts[i + 1] })
        } catch (err) {
    ...
    it('Register public key of the last voter', async () => {
        ...
        _merkleProof = usersMerkleTree.getHexProof(accounts[i + 1])
        const sender = accounts[i + 1]
        tx = await eVoteInstance.register(sender, v, r, s, data[i - 1].publicKey, data[i - 1].publicKeyProof.a, data[i - 1].publicKeyProof.b, data[i - 1].publicKeyProof.c, _merkleProof, { from: accounts[i + 1] })
        gasUsedRegister.push(tx.receipt.gasUsed.toString())
    ...
    it('Cast valid encrypted votes except the last one', async () => {
        ...
        for (let i = 0; i < data.length - 1; i++) {
            const sender = accounts[i + 1]
            tx = await eVoteInstance.castVote(sender, v, r, s, data[i].encryptedVote, data[i].Idx, data[i].encryptedVoteProof.a, data[i].encryptedVoteProof.b, data[i].encryptedVoteProof.c, { from: accounts[i + 1] })
        }
    ...
    it('Throw an error if elligable user provides invalid encrypted vote', async () => {
        ...
        try {
            const sender = accounts[i + 1];
            await eVoteInstance.castVote(sender, v, r, s, data[i - 1].encryptedVote, data[i].Idx, data[i].encryptedVoteProof.a, data[i].encryptedVoteProof.b, data[i].encryptedVoteProof.c, { from: accounts[i + 1] })
        } catch (err) {
    ...
    it('Update the encrypted vote of the last voter', async () => {
        i = data.length - 1;
        const sender = accounts[i + 1];
        tx = await eVoteInstance.castVote(sender, v, r, s, data[i].encryptedVote, data[i].Idx, data[i].encryptedVoteProof.a, data[i].encryptedVoteProof.b, data[i].encryptedVoteProof.c, { from: accounts[0] })
